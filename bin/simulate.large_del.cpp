    ...
    vector<int> unit_data)
{
	...
	else if (mutated_sequence.length() > 100000000) {
	...
	int ins_count = 0; attempt = 0; max_attempt = num_insertions * 50;
	    ...
	    if (mutated_sequence.length() + ins_size > 100000000)
		    continue;
        ...
        // ========== LARGE DELETIONS ==========
        int num_large_deletions = poisson_sample(mutated_sequence.length() * 3.04e-11);
	//int num_large_deletions = 0;
        ...
        while (ldel_count < num_large_deletions && attempt < max_attempt) {
            ...
            int idx = random_int(0, mutated_sequence.length() - 1);
            int copy_num = max(1, poisson_sample(2809.0));
